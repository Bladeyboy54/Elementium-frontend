export const addCurrency = async (args) => {
  ...
  let userId, // <-- Gets the user ID from the args
    amount, // <-- This will be the amount "purchased" by teh user for the top-up
    currency = args; //<-- This specifies wich currency the user is adding to
  ...
  switch (currency) { // <-- now we set up a switch to determine which currency is going to be tpo-upped
    case "h2":
      balance_h2 += amount;
      break;
    case "li":
  ...
  const payload = { //<- now the payload can look the same and will contain the old balances of the other currencies and the new balance for the top-upped currency depending on the swtich.
    balance_h2,
    balance_li,
    balance_pd,
    balance_xe,
